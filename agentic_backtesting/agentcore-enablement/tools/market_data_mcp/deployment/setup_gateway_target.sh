...
echo "📋 Command: agentcore gateway create-mcp-gateway-target --gateway-arn \"$GATEWAY_ARN\" --gateway-url \"$GATEWAY_URL\" --role-arn \"$GATEWAY_ROLE_ARN\" --region \"$REGION\" --name \"$TARGET_NAME\" --target-type \"lambda\" --target-payload \"$(cat lambda-target-config.json)\""
