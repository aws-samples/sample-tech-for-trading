...
async function processBacktest(jobId: string, strategyInput: any) {
  ...
  try {
    ...
    // Paper mode: send the full payload (pdf_base64 + params) to the paper agent.
    // Fallback: plain strategy input wrapped as a prompt.
    let payload: any;
    if (strategyInput.pdf_base64) {
      payload = strategyInput;
      console.log('========================================');
      console.log(`[AgentCore] PAPER MODE: ${strategyInput.paper_name} | ` +
        `symbol=${strategyInput.stock_symbol} window=${strategyInput.backtest_window} | ` +
        `pdf=${Math.round(strategyInput.pdf_base64.length / 1024)}KB (base64)`);
      console.log('========================================');
    } else {
      payload = { prompt: `how is the strategy performance: ${JSON.stringify(strategyInput)}` };
      console.log('========================================');
    ...
    const extractedStrategy = result.extracted_strategy || null;

    if (extractedStrategy) {
      console.log('[AgentCore] Extracted strategy from paper:', JSON.stringify(extractedStrategy));
    }
    ...
    // Strip the large pdf_base64 before storing/logging the job result
    const { pdf_base64, ...strategyInputSansPdf } = strategyInput;
    ...
    const completeResult = {
      ...
      data: {
        ...
        strategyInput: strategyInputSansPdf,
        strategyCode,
        trades,
